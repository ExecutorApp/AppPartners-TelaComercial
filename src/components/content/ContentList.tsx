...
const InlineProgressBar: React.FC<{ progress: number; onSeek?: (fraction: number) => void; style?: any }> = ({ progress, onSeek, style }) => {
  ...
  return (
    <RNView
      style={[{ height: 8, position: 'relative' } as any, style]}
      onLayout={(e) => setWidth(e.nativeEvent.layout.width)}
      onStartShouldSetResponder={() => true}
      onResponderGrant={(e) => handleSeek(e.nativeEvent.locationX)}
      onResponderMove={(e) => handleSeek(e.nativeEvent.locationX)}
    >
      <RNView style={{ position: 'absolute', left: 0, top: 3, right: 0, height: 3, borderRadius: 4, backgroundColor: '#9FCAFF' } as any} />
      <RNView style={{ position: 'absolute', left: 0, top: 3, height: 3, borderRadius: 4, width: fillWidth, backgroundColor: '#1777CF' } as any} />
      <RNView style={{ position: 'absolute', top: 0, width: 8, height: 8, borderRadius: 4, left: Math.max(0, fillWidth - 4), backgroundColor: '#1777CF' } as any} />
    </RNView>
  );
};
