...
import CommercialMenu from './3.2.InformationGroup-SalesFlow-Activities-Commercial-Menu';
...
type CategoryType = 'comercial' | 'operacional' | 'controladoria' | 'financeiro';

// ========================================
// CATEGORIAS DO FILTRO
// Opcoes de filtro de categorias na aba de atividades
// ========================================
const CATEGORIES = [
  { id: 'comercial' as const, label: 'Comercial' },
  { id: 'operacional' as const, label: 'Operacional' },
  { id: 'controladoria' as const, label: 'Controladoria' },
...
type Props = {
  activities: ActivityItem[];                   // Lista de atividades
  selectedActivityId: string | null;            // ID da atividade selecionada (gerenciado pelo pai)
  onActivitySelect: (activityId: string) => void; // Callback para selecionar atividade
  selectedProduct: ProductItem | null;          // Produto selecionado
  selectedPhase: PhaseItem | null;              // Fase selecionada
  selectedProductIndex: number;                 // Indice do produto selecionado
  selectedPhaseIndex: number;                   // Indice da fase selecionada
  totalProducts: number;                        // Total de produtos
  totalPhases: number;                          // Total de fases
  contextCardVisible: boolean;                  // Visibilidade do container de contexto
...
const CustomersInformationGroupSalesFlowActivities: React.FC<Props> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<CategoryType>('comercial');
  ...
  // Calcula indice e total de atividades selecionadas
  const selectedActivity = activities.find(a => a.id === selectedActivityId);
  const selectedActivityIndex = selectedActivity ? activities.findIndex(a => a.id === selectedActivityId) + 1 : 0;
  ...
  return (
    <View style={styles.page}>
      ...
      {/* Container de Filtro de Categorias */}
          ...
          {CATEGORIES.map((cat) => (
            <TouchableOpacity
              ...
              style={[
                ...
                selectedCategory === cat.id && styles.categoryFilterItemActive,
              ]}
      ...
      {/* Lista de Atividades - Comercial (com menu 3 pontinhos) */}
      {selectedCategory === 'comercial' && (
        <CommercialMenu
          activities={activities.map((a, idx) => ({
            id: a.id,
            title: a.title,
            count: `${String(idx + 1).padStart(2, '0')}/${String(activities.length).padStart(2, '0')}`,
            status: a.status,
          }))}
      ...
      {/* Lista de Atividades - Outras categorias (com icone de chat) */}
      {selectedCategory !== 'comercial' && (
...
const styles = StyleSheet.create({
  ...
  categoryFilterContainer: {
    alignSelf: 'stretch', //.........Estica para largura total
    paddingHorizontal: 18, //........Margem horizontal (igual ao statusBlock)
    marginTop: 10, //................Margem superior
    marginBottom: 10, //.............Margem inferior
  },
  categoryFilterWrapper: {
    height: 40, //..................Altura do container
    padding: 4, //.................Espacamento interno
    backgroundColor: COLORS.background, //...Cor de fundo cinza
    borderRadius: 8, //.............Arredondamento das bordas
    borderWidth: 0.3, //............Largura da borda
  ...
  categoryFilterItem: {
    flex: 1, //....................Cada item ocupa espaco igual
    height: 32, //..................Altura do item
  ...
  categoryFilterItemActive: {
    backgroundColor: COLORS.primary, //...Cor de fundo azul quando ativo
  },
  categoryFilterText: {
    color: COLORS.textPrimary, //...Cor do texto escuro
    fontSize: 12.5, //................Tamanho da fonte
    fontFamily: 'Inter_400Regular', //...Fonte regular
  },
  categoryFilterTextActive: {
    color: COLORS.white, //.........Cor do texto branco quando ativo
  },
  // ========================================
  // CONTAINER DE STATUS
  // ========================================
