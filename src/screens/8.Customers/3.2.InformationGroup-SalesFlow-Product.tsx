...
// Icone de Produtos (para container de contexto)
const ProductsContextIcon = ({ size = 18 }: { size?: number }) => (
...
// Icone de Fases (para container de contexto)
const PhasesContextIcon = ({ size = 17 }: { size?: number }) => (
...
const CustomersInformationGroupSalesFlowProducts: React.FC<Props> = ({
  ...
  const [selectedProductForMenu, setSelectedProductForMenu] = useState<ProductItem | null>(null);

  // Abre modal de opcoes ao clicar no menu
  const handleMenuPress = (product: ProductItem) => {
  ...
  return (
    <View style={styles.page}>
      ...
      <StatusActivitiesModal
        ...
        activity={null}
        productInfo={{
          id: selectedProductForMenu?.id || '',
          name: selectedProductForMenu?.title || '',
          status: selectedProductForMenu?.status || 'not_started',
        }}
        phaseInfo={{ id: '', name: '', status: 'not_started', productId: '' }}
...
const styles = StyleSheet.create({
  ...
  contextCard: {
    ...
    paddingHorizontal: 10,            // Espacamento horizontal interno
    paddingVertical: 10,              // Espacamento vertical interno
    borderRadius: 4,                  // Bordas arredondadas
    marginHorizontal: 18,             // Margem horizontal externa
    marginTop: 0,                     // Margem superior externa
    marginBottom: 20,                 // Margem inferior externa
    borderWidth: 0.5,                 // Largura da borda
    borderColor: COLORS.border,       // Cor da borda
    position: 'relative' as const,    // Posicao relativa para linha tracejada
  },
  contextRow: {
    flexDirection: 'row' as const,    // Layout horizontal
    alignItems: 'center' as const,    // Alinha verticalmente ao centro
    gap: 10,                          // Espaco entre elementos
  },
  contextIconContainer: {
    width: 18,                        // Largura do container do icone
    height: 15,                       // Altura do container do icone
    justifyContent: 'center' as const,  // Centraliza verticalmente
    alignItems: 'center' as const,    // Centraliza horizontalmente
  },
  contextContent: {
    flex: 1,                          // Ocupa espaco disponivel
    flexDirection: 'row' as const,    // Layout horizontal
    justifyContent: 'space-between' as const, // Espaco entre elementos
    alignItems: 'center' as const,    // Alinha verticalmente ao centro
  },
  contextLabel: {
  ...
  contextCount: {
    ...
    textAlign: 'right' as const,      // Alinha a direita
  },
  contextDividerContainer: {
    paddingLeft: 27,                  // Espaco esquerdo para alinhar com texto
    paddingTop: 10,                   // Espaco acima da divisoria
    paddingBottom: 10,                // Espaco abaixo da divisoria
  },
  contextDivider: {
    height: 0.5,                      // Altura da linha divisoria
    backgroundColor: COLORS.border,   // Cor da linha divisoria
  },
