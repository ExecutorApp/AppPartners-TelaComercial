...
// Interface para dados completos de uma condição
...
interface DiscountModalConditionProps {
  visible: boolean;
  onClose: () => void;
  onApply?: (condicao: CondicaoData) => void;
  initialPage?: number;
}

// ============================================
// DADOS MOCKADOS DAS CONDIÇÕES
// ============================================

// Dados mockados para demonstração das 3 abas
const condicoesMock: CondicaoData[] = [
  {
    id: '10050232',
    numero: 1,
    total: 3,
    ...
    descontoTotalCliente: { prolaborePercent: '0%', prolaboreValor: 'R$ 00,00', exitoPercent: '0%', exitoValor: 'R$ 00,00', totalPercent: '0%', totalValor: 'R$ 00,00' },
    reducaoTotalPremiacao: { prolaborePercent: '0%', prolaboreValor: 'R$ 00,00', exitoPercent: '0%', exitoValor: 'R$ 00,00', totalPercent: '0%', totalValor: 'R$ 00,00' },
  },
  {
    id: '20050232',
    numero: 2,
...
const ExitoIcon: React.FC = () => (
  <Svg width={16} height={20} viewBox="0 0 16 20" fill="none">
    <Path
      d="M11.286 4.37973C11.2925 4.3727 11.2991 4.3657 11.3055 4.35871C11.7008 3.93355 11.3978 3.24219 10.8171 3.24219H4.41727C3.83656 3.24219 3.53359 3.93355 3.92875 4.35871L3.94832 4.37973C4.54254 5.015 6.77918 6.90234 6.80797 6.92703C7.27367 7.32645 7.96066 7.32645 8.42633 6.92703C8.45516 6.9023 10.6918 5.015 11.286 4.37973Z"
      fill="#FCFCFC"
    />
    <Path
...
const DiscountModalCondition: React.FC<DiscountModalConditionProps> = ({
  visible,
  onClose,
  onApply,
  initialPage,
  ...
  // Dados da condição atual baseado na página selecionada
  ...
  const calculateCondicao = (input: CondicaoData): CondicaoData => {
    const produtoValor = parseBRL(input.valorProduto);
    const honorarios = input.honorarios.map((h) => {
      const hTotal = parseBRL(h.valor);
      let descontoTotal = 0;
      const metodos = h.metodos.map((m) => {
        const v = parseBRL(m.valor);
        const p = parsePercent(m.descontoPercentNovo);
        const desconto = (v * p) / 100;
        return {
          ...m,
          descontoPercentNovo: formatPercentUpTo6(p),
          descontoValorNovo: formatBRL(desconto),
        };
      });
      metodos.forEach((m) => {
        descontoTotal += parseBRL(m.descontoValorNovo || 'R$ 0,00');
      });
      const descontoPercentEfetivo = hTotal > 0 ? (descontoTotal / hTotal) * 100 : 0;
      return {
        ...h,
        metodos,
        totalDescontoPercentNovo: formatPercentUpTo6(descontoPercentEfetivo),
        totalDescontoValorNovo: formatBRL(descontoTotal),
      };
    });
    const pro = honorarios.find((x) => x.tipo === 'prolabore');
    const ex = honorarios.find((x) => x.tipo === 'exito');
    const proTotal = pro ? parseBRL(pro.valor) : 0;
    const exTotal = ex ? parseBRL(ex.valor) : 0;
    const proDesc = pro ? parseBRL(pro.totalDescontoValorNovo || 'R$ 0,00') : 0;
    const exDesc = ex ? parseBRL(ex.totalDescontoValorNovo || 'R$ 0,00') : 0;
    ...
    const reducaoPercentStr = formatPercentUpTo6(reducaoPercent);
    const reducaoValorStr = formatBRL(reducaoValor);

    const honorariosComReducao = honorarios.map((h) => ({
      ...h,
      metodos: h.metodos.map((m) => ({
        ...m,
        reducaoPercentNovo: reducaoPercentStr,
        reducaoValorNovo: reducaoValorStr,
      })),
      totalReducaoPercentNovo: reducaoPercentStr,
      totalReducaoValorNovo: reducaoValorStr,
    }));

    const descontoTotalCliente = {
      prolaborePercent: formatPercentUpTo6(proTotal > 0 ? (proDesc / proTotal) * 100 : 0),
      prolaboreValor: formatBRL(proDesc),
      exitoPercent: formatPercentUpTo6(exTotal > 0 ? (exDesc / exTotal) * 100 : 0),
      exitoValor: formatBRL(exDesc),
      totalPercent: formatPercentUpTo6(descontoPercentGeral),
      totalValor: formatBRL(descontoGeral),
    };

    const reducaoTotalPremiacao = {
      prolaborePercent: reducaoPercentStr,
      prolaboreValor: reducaoValorStr,
      exitoPercent: reducaoPercentStr,
      exitoValor: reducaoValorStr,
      totalPercent: reducaoPercentStr,
      totalValor: reducaoValorStr,
    };

    return {
      ...input,
      honorarios: honorariosComReducao,
      descontoTotalCliente,
      reducaoTotalPremiacao,
    };
  };

  const currentCondicao = React.useMemo(() => calculateCondicao(condicoesMock[currentPage]), [currentPage]);
  ...
  const goToNextPage = (): void => {
    if (currentPage < condicoesMock.length - 1) {
      setCurrentPage(currentPage + 1);
    }
  };
  ...
  // Componente para renderizar linha com valores
  ...
  const MetodoPagamentoCard: React.FC<{
    ...
    <View style={styles.metodoPagamentoCard}>
      ...
      <ValueLine
        label="Desconto"
        percent={metodo.descontoPercentNovo || '0%'}
        value={metodo.descontoValorNovo || 'R$ 00,00'}
      />
      ...
      <ValueLine
        ...
        percent={metodo.reducaoPercentNovo || '0%'}
        value={metodo.reducaoValorNovo || 'R$ 00,00'}
      />
    </View>
  );
  ...
  const TotalHonorarioCard: React.FC<{
    ...
    <View style={styles.totalHonorarioCard}>
      ...
      <ValueLine
        label="Desconto"
        percent={honorario.totalDescontoPercentNovo || '0%'}
        value={honorario.totalDescontoValorNovo || 'R$ 00,00'}
      />
      ...
      <ValueLine
        label="Redução de premiação"
        percent={honorario.totalReducaoPercentNovo || '0%'}
        value={honorario.totalReducaoValorNovo || 'R$ 00,00'}
      />
    </View>
  );
    ...
    <View style={styles.honorarioSection}>
      ...
      {isExpanded && (
        <>
          ...
          {/* Card de Total */}
          <TotalHonorarioCard honorario={honorario} />
        </>
      )}
    </View>
  );
        ...
        <View style={styles.descontoClienteSection}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.prolaborePercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.prolaboreValor}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.descontoTotalRow}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.exitoPercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.exitoValor}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.descontoTotalRow}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.totalPercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.descontoTotalCliente.totalValor}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.descontoClienteSection}>
          ...
          <View style={styles.descontoTotalRow}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.prolaborePercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.prolaboreValor}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.descontoTotalRow}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.exitoPercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.exitoValor}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.descontoTotalRow}>
            ...
            <View style={styles.descontoTotalValuesRow}>
              <View style={styles.descontoTotalPercentLeft}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.totalPercent}
                </Text>
              </View>
              <View style={styles.descontoTotalMoneyRight}>
                <Text style={styles.descontoTotalValue}>
                  {condicao.reducaoTotalPremiacao.totalValor}
                </Text>
              </View>
            </View>
          </View>
        ...
        >
            ...
            <View style={styles.headerTitleColumn}>
              ...
              <View style={styles.headerTitleRow}>
                <Text style={styles.headerTitle}>
                  Condição {formatNumber(currentCondicao.numero)}/
                  {formatNumber(currentCondicao.total)}
                </Text>
              </View>

              {/* ID da condição */}
              <View style={styles.headerIdRow}>
                <Text style={styles.headerId}>ID: {currentCondicao.id}</Text>
              </View>
            </View>
          ...
          <View style={styles.paginationContainer}>
            ...
            <TouchableOpacity
              ...
              disabled={currentPage === condicoesMock.length - 1}
              activeOpacity={0.7}
            >
              <View
                style={{
                  opacity: currentPage === condicoesMock.length - 1 ? 0.4 : 1,
                }}
              >
                <ArrowRightIcon />
              </View>
          ...
          >
            ...
            <DetalhesVendaSection detalhes={currentCondicao.detalhesVenda} />
            ...
            <View style={styles.formaPagamentoContainer}>
              ...
              <View style={styles.formaPagamentoInnerBorder}>
                ...
                {currentCondicao.honorarios.map((honorario, index) => (
                  <HonorarioSection
                ...
                <DescontoTotalSection condicao={currentCondicao} />
              </View>
            </View>
          </ScrollView>

          {/* Botão Aplicar desconto - FIXO NO RODAPÉ */}
          <View style={styles.aplicarDescontoContainer}>
            <TouchableOpacity
              style={styles.aplicarDescontoButton}
              onPress={() => onApply && onApply(currentCondicao)}
              activeOpacity={0.7}
            >
              <Text style={styles.aplicarDescontoButtonText}>
                Aplicar desconto
              </Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  // Divisor entre seções do desconto total
  ...
  // Container do botão Aplicar desconto fixo no rodapé
  aplicarDescontoContainer: {
    marginHorizontal: 16,
    marginTop: 12,
    marginBottom: 17,
  },
  ...
  // Texto do botão Aplicar desconto
  aplicarDescontoButtonText: {
    color: '#FCFCFC',
    fontSize: 14,
    fontFamily: 'Inter_500Medium',
    textAlign: 'center',
...
export default DiscountModalCondition;
