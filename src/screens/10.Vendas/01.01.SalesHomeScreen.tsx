...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Dimensions, Image, ViewStyle, TextStyle, FlatList, Platform, Alert, Linking } from 'react-native';
import { Layout } from '../../constants/theme';
import Svg, { Path, Rect } from 'react-native-svg';
import { SvgXml } from 'react-native-svg';
import CardMenu from './01.01.SalesHomeScreen-Modal-Options';
...
const SalesHomeScreen: React.FC = () => {
  ...
  const route = useRoute<RouteProp<RootStackParamList, 'SalesHome'>>();
  React.useEffect(() => { console.log('[SalesHome] mount'); }, []);
  React.useEffect(() => { try { console.log('[SalesHome] route.params', route.params); } catch {} }, [route.params]);
  React.useEffect(() => {
    const shouldAuto = !!route.params?.autoOpenDiscount;
    if (shouldAuto) {
      // Reseta a flag de navegação ao voltar da tela DiscountSales
      setNavigatingToDiscount(false);
      setIsEditingSchedule(false);
      setNewAppointmentStep(6);
      setNewAppointmentVisible(true);
    }
  }, [route.params?.autoOpenDiscount]);
  ...
  const requestLeaveSchedule = () => {
    // Não mostra o modal de confirmação se estiver navegando para DiscountSales
    if (navigatingToDiscount) return;
    setLeaveModalVisible(true);
  };
  
  // Oculta temporariamente o modal principal para permitir navegação
  // Não reseta o estado para permitir voltar ao fluxo
  const hideModalForNavigation = () => {
  ...
  const handleSchedule = async (date: Date, slots: SlotPair[], client?: { id: string; photoKey?: string; photoUri?: string | null } | null, extra?: any) => {
    ...
    const hourTextNow = `${hh}:${mm}`;
    const typeLabelRaw = String((flowSummaries?.[5] ?? extra?.agendaType ?? '') || '');
    const typeLabelNorm = typeLabelRaw.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    const summ6 = String(flowSummaries?.[6] ?? '');
    const proMatch = summ6.match(/prolabore\s*:\s*R\$\s*([0-9\.,]+)/i);
    const exMatch = summ6.match(/(?:êxito|exito)\s*:\s*R\$\s*([0-9\.,]+)/i);
    const parsePtNumber = (s: string) => {
    ...
    let amountNum = 0;
    if (typeLabelNorm.includes('prolabore') && (typeLabelNorm.includes('exito'))) {
      const proVal = proMatch ? parsePtNumber(proMatch[1]) : 40000;
      const exVal = exMatch ? parsePtNumber(exMatch[1]) : 95000;
      amountNum = Math.max(proVal + exVal, 0);
    } else if (typeLabelNorm.includes('prolabore')) {
      amountNum = 120000;
    } else if (typeLabelNorm.includes('exito')) {
      amountNum = 160000;
    } else {
      amountNum = 120000;
    }
    const amountText = (() => {
      try { return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(amountNum); } catch { return `R$ ${String(amountNum)}`; }
    })();
  ...
  const renderSmallClockIcon = () => (
    <Svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      <Path d="M6.84596 3.18003C6.8113 2.88162 6.5577 2.65 6.25 2.65C5.91863 2.65 5.65 2.91863 5.65 3.25V6.4L5.65514 6.47849C5.68231 6.68484 5.81538 6.86476 6.00951 6.94969L8.40951 7.99969L8.47523 8.02404C8.76251 8.1119 9.07636 7.97239 9.19969 7.69049L9.22404 7.62477C9.3119 7.33749 9.17239 7.02364 8.89049 6.90031L6.85 6.0076V3.25L6.84596 3.18003Z" fill="#91929E"/>
      <Path fillRule="evenodd" clipRule="evenodd" d="M6.25 0.25C2.93629 0.25 0.25 2.93629 0.25 6.25C0.25 9.56371 2.93629 12.25 6.25 12.25C9.56371 12.25 12.25 9.56371 12.25 6.25C12.25 2.93629 9.56371 0.25 6.25 0.25ZM6.25 1.45C8.90097 1.45 11.05 3.59903 11.05 6.25C11.05 8.90097 8.90097 11.05 6.25 11.05C3.59903 11.05 1.45 8.90097 1.45 6.25C1.45 3.59903 3.59903 1.45 6.25 1.45Z" fill="#91929E"/>
      <Path d="M6.84596 3.18003C6.8113 2.88162 6.5577 2.65 6.25 2.65C5.91863 2.65 5.65 2.91863 5.65 3.25V6.4L5.65514 6.47849C5.68231 6.68484 5.81538 6.86476 6.00951 6.94969L8.40951 7.99969L8.47523 8.02404C8.76251 8.1119 9.07636 7.97239 9.19969 7.69049L9.22404 7.62477C9.3119 7.33749 9.17239 7.02364 8.89049 6.90031L6.85 6.0076V3.25L6.84596 3.18003Z" stroke="#FCFCFC" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round"/>
      <Path fillRule="evenodd" clipRule="evenodd" d="M6.25 0.25C2.93629 0.25 0.25 2.93629 0.25 6.25C0.25 9.56371 2.93629 12.25 6.25 12.25C9.56371 12.25 12.25 9.56371 12.25 6.25C12.25 2.93629 9.56371 0.25 6.25 0.25ZM6.25 1.45C8.90097 1.45 11.05 3.59903 11.05 6.25C11.05 8.90097 8.90097 11.05 6.25 11.05C3.59903 11.05 1.45 8.90097 1.45 6.25C1.45 3.59903 3.59903 1.45 6.25 1.45Z" stroke="#FCFCFC" strokeWidth="0.5" strokeLinecap="round" strokeLinejoin="round"/>
    </Svg>
  );
  ...
  const renderAppointmentCard = (apt: AppointmentItem) => {
    ...
    const ddmmyy = formatDDMMYY(d);
    const hourText = displayHour(firstSlot);
    const visibleHour = (apt as any)?.hour || hourText || '00:00';
    const priceLabel = ((apt as any)?.price ?? (apt as any)?.amount ?? null) ? String((apt as any)?.price ?? (apt as any)?.amount) : '—';
    const rawStatus = String(((apt as any)?.status ?? '')).trim();
    const statusLabel = rawStatus.length ? rawStatus : 'Aguardando pagamento';
    ...
    // Monta estilos do tile esquerdo conforme referência do anexo
    const tileStyles: ViewStyle[] = [styles.newLeftTile];
    ...
    return (
      ...
      >
        ...
        <View style={tileStyles}>
          {/* Bloco: foto + data e hora */}
          <View style={styles.newLeftTileContent}>
            <View style={styles.newLeftTileImageWrapper}>
              <View style={styles.newLeftTilePhotoArea}><View style={styles.newLeftTileImageContainer}>
                <Image
                  style={styles.newLeftTileImage}
                  source={resolveClientPhotoSource(apt)}
                  resizeMode="cover"
                />
              </View>
              <View style={styles.newIndicatorOverlay}>
                <Svg width="30" height="30" viewBox="0 0 30 30" fill="none">
                  <Rect width="30" height="30" rx="15" fill="#FCFCFC"/>
                  <Rect width="30" height="30" rx="15" stroke="#E5E7EB"/>
                  <Path d="M18.5357 12.4643C18.7915 12.4643 19.0479 12.3667 19.2427 12.1714L22.0713 9.34273C22.462 8.95258 22.462 8.3193 22.0713 7.92867C21.6807 7.53805 21.0469 7.53805 20.6573 7.92867L17.8286 10.7573C17.438 11.1475 17.438 11.7812 17.8286 12.1714C18.0234 12.3667 18.2798 12.4643 18.5357 12.4643ZM24 14H20C19.4477 14 19 14.4478 19 15C19 15.5523 19.4477 16 20 16H24C24.5523 16 25 15.5523 25 15C25 14.4478 24.5523 14 24 14ZM17.8286 19.2427L20.6573 22.0713C20.8521 22.2666 21.1084 22.3643 21.3643 22.3643C21.6202 22.3643 21.876 22.2666 22.0713 22.0713C22.462 21.6807 22.462 21.0474 22.0713 20.6573L19.2427 17.8286C18.8525 17.438 18.2188 17.438 17.8286 17.8286C17.438 18.2188 17.438 18.8525 17.8286 19.2427ZM14 24C14 24.5523 14.4478 25 15 25C15.5523 25 16 24.5523 16 24V20C16 19.4477 15.5523 19 15 19C14.4478 19 14 19.4477 14 20V24ZM9.34277 22.0713L12.1714 19.2427C12.562 18.8525 12.562 18.2187 12.1714 17.8286C11.7812 17.438 11.1475 17.438 10.7573 17.8286L7.92871 20.6572C7.53809 21.0474 7.53809 21.6807 7.92871 22.0713C8.12402 22.2666 8.37988 22.3643 8.63574 22.3643C8.8916 22.3643 9.14793 22.2666 9.34277 22.0713ZM5 15C5 15.5523 5.44773 16 6 16H10C10.5522 16 11 15.5523 11 15C11 14.4478 10.5522 14 10 14H6C5.44773 14 5 14.4478 5 15ZM9.34277 7.92871C8.95312 7.53809 8.31934 7.53809 7.92871 7.92871C7.53809 8.31934 7.53809 8.95262 7.92871 9.34277L10.7573 12.1714C10.9521 12.3667 11.2085 12.4644 11.4644 12.4644C11.7202 12.4644 11.9766 12.3667 12.1714 12.1714C12.562 11.7812 12.562 11.1475 12.1714 10.7573L9.34277 7.92871ZM15 11C15.5523 11 16 10.5522 16 10V6C16 5.44773 15.5523 5 15 5C14.4478 5 14 5.44773 14 6V10C14 10.5522 14.4478 11 15 11Z" fill="#1777CF"/>
                </Svg>
              </View>
            </View></View>
            <View style={styles.newBottomInfo}>
              <View style={styles.newDateContainer}>
                <Text style={styles.newDateText}>{ddmmyy}</Text>
              </View>
              <View style={styles.newDivider} />
              <View style={styles.newHourContainer}>
                {renderSmallClockIcon()}
                <Text style={styles.newHourText}>{visibleHour}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.rightContent}>
          ...
          <View style={styles.divider} />
          <Text style={styles.fieldValue}>{priceLabel}</Text>

          <View style={styles.divider} />
          <Text style={[styles.statusText, statusIsConfirmed ? styles.statusTextGreen : styles.statusTextBlue, { fontSize: 14 }]}>{statusLabel}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <ModalShell
        ...
        onCloseImmediate={confirmLeaveSchedule}
        onNavigateToDiscountSalesScreen={() => {
          try { console.log('[SalesHome] navigate to DiscountCustomer'); } catch {}
          // Seta flag para bloquear modal de confirmação
          setNavigatingToDiscount(true);
          // Fecha o modal principal
          setNewAppointmentVisible(false);
          // Navega para a tela de descontos
          setTimeout(() => {
            navigation.navigate(ScreenNames.DiscountCustomer as any);
            // Reseta a flag após navegação
            setTimeout(() => setNavigatingToDiscount(false), 500);
          }, 50);
        }}
        autoOpenDiscount={!!route.params?.autoOpenDiscount}
      ...
      <CardMenu
        ...
        onViewDetails={() => { if (selectedAppointment) openAppointmentDetails(selectedAppointment); }}
