...
import { View, Text, StyleSheet, Modal, TouchableOpacity, Platform, ScrollView, Dimensions } from 'react-native';
...
interface Props {
  ...
  selected?: number;
  onCancel?: () => void;
  onSave?: (n: number, date: Date) => void;
}
...
const RadioDot: React.FC<{ selected?: boolean }> = ({ selected }) => (
  <Svg width={20} height={20} viewBox="0 0 20 20" fill="none">
    {selected ? (
      ...
      <Circle cx={10} cy={10} r={9} stroke="#6F7DA0" strokeWidth={1.5} fill="none" />
    )}
  </Svg>
);
...
const CreditCardInstallmentModal: React.FC<Props> = ({ visible, baseAmount, selected = 1, onCancel, onSave }) => {
  ...
  const [viewDate, setViewDate] = useState<Date>(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));
  const [selectedDate, setSelectedDate] = useState<Date>(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));
  ...
  const monthLabel = `${new Intl.DateTimeFormat('pt-BR', { month: 'long' }).format(viewDate)} - ${viewDate.getFullYear()}`;
  ...
  const buildCells = () => {
    const arr: Array<{ day: number; inMonth: boolean; date: Date }>[] = [];
    const totalCells = 42;
    const startPrev = prevMonthDays - firstWeekday + 1;
    for (let i = 0; i < totalCells; i++) {
      ...
      if (i < firstWeekday) {
        dayNum = startPrev + i;
        inMonth = false;
        dateObj = new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, dayNum);
      ...
      arr[row].push({ day: dayNum, inMonth, date: dateObj });
    }
    return arr;
  };
  const monthGrid = buildCells();
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}>
      <View style={styles.overlay}>
        <View style={[
          styles.card,
          {
            height: Math.max(0, Dimensions.get('window').height - 30),
            width: Math.max(0, Dimensions.get('window').width - 30),
          },
        ]}>
          <View style={styles.headerRow}>
            ...
            <TouchableOpacity style={styles.closeButton} onPress={onCancel} accessibilityRole="button" accessibilityLabel="Fechar">
              <Svg width={13} height={12} viewBox="0 0 14 14" fill="none">
                <Path d="M1 1L13 13" stroke="#3A3F51" strokeWidth={2} strokeLinecap="round" />
                <Path d="M13 1L1 13" stroke="#3A3F51" strokeWidth={2} strokeLinecap="round" />
              </Svg>
            </TouchableOpacity>
          </View>
          <View style={styles.listWrap}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.calendarIntroRow}>
                <View style={styles.calendarIconBox}>
                  <Svg width={17} height={17} viewBox="0 0 17 17" fill="none">
                    <Path d="M0.5 7.08824H16.5M4.5 2.38235V0.5M12.5 2.38235V0.5M2.5 16.5H14.5C15.6046 16.5 16.5 15.6572 16.5 14.6176V4.26471C16.5 3.22511 15.6046 2.38235 14.5 2.38235H2.5C1.39543 2.38235 0.5 3.22511 0.5 4.26471V14.6176C0.5 15.6572 1.39543 16.5 2.5 16.5Z" stroke="#FCFCFC" strokeLinecap="round" strokeLinejoin="round" />
                  </Svg>
                </View>
                <Text style={styles.calendarIntroText}>Escolha a data de vencimento da primeira parcela</Text>
                <TouchableOpacity
                  style={styles.calendarSmallButton}
                  onPress={() => setCalendarExpanded((v) => !v)}
                  accessibilityRole="button"
                  accessibilityLabel={calendarExpanded ? 'Recolher calendário' : 'Expandir calendário'}
                >
                  <Svg width={14} height={14} viewBox="0 0 12 12" fill="none">
                    <Path d="M2 6H10" stroke="#1777CF" strokeWidth={2} strokeLinecap="round" />
                    {calendarExpanded ? null : (
                      <Path d="M6 2V10" stroke="#1777CF" strokeWidth={2} strokeLinecap="round" />
                    )}
                  </Svg>
                </TouchableOpacity>
              </View>
              ...
              {calendarExpanded ? (
                <>
                  <View style={styles.monthHeaderRow}>
                    <TouchableOpacity style={styles.monthArrowButton} onPress={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))} accessibilityRole="button" accessibilityLabel="Mês anterior">
                      <Svg width={7} height={10} viewBox="0 0 7 10" fill="none">
                        <Path d="M6.18333 1.175L2.35833 5L6.18333 8.825L5 10L0 5L5 0L6.18333 1.175Z" fill="#1777CF" />
                      </Svg>
                    </TouchableOpacity>
                    <View style={styles.monthHeaderCenter}><Text style={styles.monthHeaderText}>{monthLabel[0].toUpperCase() + monthLabel.slice(1)}</Text></View>
                    <TouchableOpacity style={styles.monthArrowButton} onPress={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 1))} accessibilityRole="button" accessibilityLabel="Próximo mês">
                      <Svg width={7} height={10} viewBox="0 0 7 10" fill="none">
                        <Path d="M6.18333 1.175L2.35833 5L6.18333 8.825L5 10L0 5L5 0L6.18333 1.175Z" fill="#1777CF" transform="translate(7,0) scale(-1,1)" />
                      </Svg>
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.dayNamesRow}>
                    {dayNames.map((name) => (
                      <View key={name} style={styles.dayNameCell}><Text style={styles.dayNameText}>{name}</Text></View>
                    ))}
                  </View>
                  ...
                  <View style={styles.monthGrid}>
                    {monthGrid.map((week, wi) => (
                      <View key={`w-${wi}`} style={styles.weekRow}>
                        {week.map((cell) => {
                          const isSelected = selectedDate.getFullYear() === cell.date.getFullYear() && selectedDate.getMonth() === cell.date.getMonth() && selectedDate.getDate() === cell.date.getDate();
                          return (
                            <TouchableOpacity
                              key={`${cell.date.getFullYear()}-${cell.date.getMonth()}-${cell.day}`}
                              style={[styles.dayCell, !cell.inMonth ? styles.dayCellMuted : null, isSelected ? styles.dayCellActive : null]}
                              onPress={() => { setSelectedDate(cell.date); setViewDate(new Date(cell.date.getFullYear(), cell.date.getMonth(), 1)); }}
                              accessibilityRole="button"
                              accessibilityLabel={`Selecionar ${formatShort(cell.date)}`}
                            >
                              <Text style={[styles.dayText, !cell.inMonth ? styles.dayTextMuted : null, isSelected ? styles.dayTextActive : null]}>{String(cell.day).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          );
                        })}
                      </View>
              ...
              <View style={styles.dateInputBox}><Text style={styles.dateInputText}>{formatShort(selectedDate)}</Text></View>

              <View style={styles.divider} />
              <View style={styles.paymentContainerRow}>
                <View style={styles.paymentContainerIcon}>
                  <Svg width={35} height={37} viewBox="0 0 35 37" fill="none">
                    <Rect width={35} height={37} rx={8} transform="matrix(-1 0 0 1 35 0)" fill="#1777CF" />
                    <Path fillRule="evenodd" clipRule="evenodd" d="M25.5 18.5C25.5 14.0817 21.9183 10.5 17.5 10.5C13.0817 10.5 9.5 14.0817 9.5 18.5C9.5 22.9183 13.0817 26.5 17.5 26.5C21.9183 26.5 25.5 22.9183 25.5 18.5ZM11.1 18.5C11.1 14.9654 13.9654 12.1 17.5 12.1C21.0346 12.1 23.9 14.9654 23.9 18.5C23.9 22.0346 21.0346 24.9 17.5 24.9C13.9654 24.9 11.1 22.0346 11.1 18.5Z" fill="#FCFCFC" />
                  </Svg>
                </View>
                <Text style={styles.paymentContainerText}>Selecione uma opção de pagamento</Text>
              </View>
              <View style={styles.divider} />
          ...
          <View style={styles.footerRow}>
            ...
            <TouchableOpacity style={[styles.saveButton]} onPress={() => onSave?.(current, selectedDate)} accessibilityRole="button" accessibilityLabel="Salvar">
...
const styles = StyleSheet.create({
  ...
  listWrap: { flex: 1, alignSelf: 'stretch', paddingHorizontal: 15, paddingTop: 15 },
  calendarIntroRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', columnGap: 10, paddingBottom: 15, paddingTop: 15  },
  calendarIconBox: { width: 35, height: 37, borderRadius: 8, borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1777CF' },
  calendarIntroText: { flex: 1, color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  calendarSmallButton: { width: 40, height: 35, borderRadius: 6, borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1777CF08' },
  calendarSmallButtonText: { color: '#1777CF', fontSize: 20, fontFamily: 'Inter_500Medium' },
  monthHeaderRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, paddingBottom: 10, paddingTop: 10 },
  monthArrowButton: { width: 40, height: 35, borderRadius: 6, backgroundColor: '#1777CF08', borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', alignItems: 'center', justifyContent: 'center' },
  monthArrowText: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  monthHeaderCenter: { flex: 1, marginHorizontal: 8, height: 35, borderRadius: 6, borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', backgroundColor: '#1777CF08', alignItems: 'center', justifyContent: 'center' },
  monthHeaderText: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  dayNamesRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', gap: 6, marginTop: 10 },
  dayNameCell: { flex: 1, alignItems: 'center' },
  dayNameText: { textAlign: 'center', color: '#7D8592', fontSize: 12, fontFamily: 'Inter_500Medium' },
  monthGrid: { marginTop: 6, gap: 6 },
  weekRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 6 },
  dayCell: { flex: 1, height: 35, borderRadius: 4, borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', backgroundColor: '#FCFCFC', alignItems: 'center', justifyContent: 'center' },
  dayCellMuted: { opacity: 0.4 },
  dayCellActive: { backgroundColor: '#1777CF', borderColor: '#1777CF' },
  dayText: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_400Regular' },
  dayTextMuted: { color: '#3A3F51' },
  dayTextActive: { color: '#FCFCFC' },
  dateInputBox: { alignSelf: 'stretch', height: 35, borderRadius: 6, borderWidth: Borders.width(0.6), borderColor: '#D8E0F0', backgroundColor: '#1777CF08', alignItems: 'center', justifyContent: 'center', marginTop: 10, marginBottom: 10 },
  dateInputText: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  paymentNoticeRow: { flexDirection: 'row', alignItems: 'center', columnGap: 10, marginTop: 10 },
  paymentContainerRow: { alignSelf: 'stretch', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', columnGap: 10, marginBottom: 15, marginTop: 15 },
  paymentContainerIcon: { width: 35, height: 37, borderRadius: 8 },
  paymentContainerText: { flex: 1, color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_400Regular' },
  row: { height: 55, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  rowLeft: { flexDirection: 'row', alignItems: 'center', columnGap: 10 },
  rowLabel: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  rowValue: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_400Regular' },
  ...
  cancelText: { color: '#3A3F51', fontSize: 14, fontFamily: 'Inter_500Medium' },
  saveButton: { flex: 1, height: 40, borderRadius: 6, backgroundColor: '#1777CF', alignItems: 'center', justifyContent: 'center' },
  saveText: { color: '#FCFCFC', fontSize: 14, fontFamily: 'Inter_500Medium' },
});
