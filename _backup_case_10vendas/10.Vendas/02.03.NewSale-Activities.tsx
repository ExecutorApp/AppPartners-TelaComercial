...
const GreenCheckIcon: React.FC = () => (
  <Svg width={15} height={11} viewBox="0 0 15 11" fill="none">
    <Path
      d="M14.3216 2.13409L6.19652 10.2589C5.95276 10.5027 5.63278 10.6254 5.31281 10.6254C4.99283 10.6254 4.67285 10.5027 4.42909 10.2589L0.366669 6.19652C-0.122223 5.70786 -0.122223 4.91776 0.366669 4.42909C0.855331 3.9402 1.6452 3.9402 2.13409 4.42909L5.31281 7.6078L12.5542 0.366669C13.0428 -0.122223 13.8327 -0.122223 14.3216 0.366669C14.8103 0.855331 14.8103 1.6452 14.3216 2.13409Z"
      fill="#1B883C"
    />
  </Svg>
);
...
export default function NewAppointmentActivities({
  visible,
  onClose,
  onBack,
  onNext,
  ...
  registerNextHandler,
  onCanProceedChange,
}: NewAppointmentActivitiesProps) {
  // ===== Estados do componente =====
  const [fullFlowVisible, setFullFlowVisible] = useState<boolean>(false);
  const [query, setQuery] = useState<string>('');
  const [selectedActivity, setSelectedActivity] = useState<string | null>('a3');
  ...
  const activities = useMemo(
    () =>
      [
        { id: 'a1', name: 'Reunião de entrada', price: 800 },
        { id: 'a2', name: 'Planejamento estratégico', price: 1200 },
        { id: 'a3', name: 'Produto completo', price: 120000 },
      ] as { id: string; name: string; price: number }[],
    []
  );

  // Filtra atividades com base na busca
  ...
  // Effect: Valida seleção quando lista filtrada ou concluídos mudam
  useEffect(() => {
    if (!filtered.length) {
    ...
    const selectedIsValid =
      !!selectedActivity && filtered.some((a) => a.id === selectedActivity) && !completedIds.has(selectedActivity);
    if (!selectedIsValid) {
  ...
  // Effect: Inicializa seleção quando modal fica visível
  useEffect(() => {
    if (visible) {
  ...
  // Alterna estado de conclusão de uma atividade
  ...
  if (embedded) {
    return (
      ...
      >
        {/* Introdução com título e subtítulo */}
        <View style={styles.introContainer}>
          ...
          <View style={styles.stepsRow}>
            {Array.from({ length: 6 }).map((_, i) => (
              <View
                key={i}
                style={[styles.stepDot, i < 3 ? styles.stepDotActive : styles.stepDotInactive]}
              />
            ))}
          </View>
        </View>
            ...
            {filtered.map((act, idx) => {
              ...
              return (
                <React.Fragment key={act.id}>
                  ...
                  >
                    {/* Ícone de seleção (radio ou check) */}
                    <TouchableOpacity
                    ...
                    <View style={styles.activityInfo}>
                      <Text style={[styles.activityName, isCompleted ? styles.activityNameStriked : null]}>
                        {act.name}
                      </Text>
                      <Text style={[styles.activityValue, isCompleted ? styles.activityValueStriked : null]}>
                        {formatBRL(act.price ?? 0)}
                      </Text>
                    </View>
                  </TouchableOpacity>
  ...
  return (
    <>
          ...
          <View style={styles.modalContainer}>
            ...
            <View style={styles.headerRow}>
              ...
              <View style={styles.headerActions}>
                {/* Botão visualizar fluxo completo */}
                <TouchableOpacity
                  style={styles.iconButton}
                  onPress={() => setFullFlowVisible(true)}
                  accessibilityRole="button"
      ...
      {/* Modal do fluxo completo */}
      <FullFlow
        visible={fullFlowVisible}
        onClose={() => setFullFlowVisible(false)}
        currentStep={4}
        summaries={summaries}
...
const styles = StyleSheet.create({
  ...
  // Container de introdução (título + subtítulo + steps)
  introContainer: {
    alignSelf: 'stretch',
    width: '100%',
    flexDirection: 'column',
  ...
  // Bloco central com título e subtítulo
  centerBlock: {
    alignItems: 'center',
    marginTop: 10,
  },

  // Container flex para lista de atividades
  contentFlex: {
    flex: 1,
    alignSelf: 'stretch',
  },
  ...
  // Linha dos indicadores de passo
  stepsRow: {
    flexDirection: 'row',
    gap: 8,
    alignSelf: 'stretch',
  ...
  // Área tocável do ícone de seleção
  selectIconTouch: {
    width: 24,
    height: 24,
    alignItems: 'center',
  ...
  // Container das informações da atividade
  activityInfo: {
    flex: 1,
    flexDirection: 'column',
    justifyContent: 'center',
  ...
  // Nome da atividade riscado (concluída)
  activityNameStriked: {
  ...
  // Valor da atividade riscado (concluída)
  ...
  // Botão do rodapé desabilitado
  footerButtonDisabled: {
    opacity: 0.5,
    ...(Platform.OS === 'web' ? ({ cursor: 'not-allowed' } as any) : {}),
  },

  // Botão do rodapé primário (azul)
  footerButtonPrimary: {
    backgroundColor: '#1777CF',
    borderColor: '#1777CF',
  },

  // Texto do botão do rodapé
  footerButtonText: {
    fontSize: 14,
    fontFamily: 'Inter_500Medium',
    color: '#3A3F51',
  ...
  // Texto do botão do rodapé primário
