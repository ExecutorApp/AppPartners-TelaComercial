...
interface PersonalizeFeesProps {
  visible: boolean;
  onClose: () => void;
  onBack: () => void;
  onNext?: () => void;
...
const parseCurrencyInput = (s: string): number => {
  ...
  const combined = integerPart + '.' + (decimalPart || '0');
...
const NewAppointmentAgendaTypesPersonalize: React.FC<PersonalizeFeesProps> = ({ visible, onClose, onBack, onNext, summaries, onSelectStep, onUpdateSummary, maxAccessibleStep = 1, transitionDirection = 'forward', embedded = false, registerNextHandler, onCanProceedChange }) => {
  const [fullFlowVisible, setFullFlowVisible] = useState<boolean>(false);

  const TOTAL = 135000;
  const [prolaboreNum, setProlaboreNum] = useState<number>(40000);
  const [exitoNum, setExitoNum] = useState<number>(95000);
  const [focusedField, setFocusedField] = useState<'prolabore' | 'exito' | null>(null);
  const [prolaboreInput, setProlaboreInput] = useState<string>(currencyNoPrefix(maskCurrencyBRL(formatBRLValue(prolaboreNum))));
  const [exitoInput, setExitoInput] = useState<string>(currencyNoPrefix(maskCurrencyBRL(formatBRLValue(exitoNum))));

  const restante = TOTAL - (prolaboreNum + exitoNum);
  const isNextDisabled = restante !== 0;
  ...
  useEffect(() => {
    const summaryStr = `Prolabore: ${formatBRL(prolaboreNum)}; Êxito: ${formatBRL(exitoNum)}`;
    onUpdateSummary?.(6, summaryStr);
  }, [prolaboreNum, exitoNum, onUpdateSummary]);
  ...
  }, [embedded, registerNextHandler, isNextDisabled, onNext]);
        ...
        <View style={styles.contentFlex}>
          <View style={styles.introContainer}>
            <View style={styles.centerBlock}>
              ...
              <Text style={styles.sectionSubtitle}>Digite os valores desejados para continuar</Text>
            </View>
          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            <View style={styles.summaryCard}>
              <View style={styles.summaryRow}>
                <View style={styles.summaryLeft}><Text style={styles.summaryLabel}>Total</Text></View>
                <View style={styles.summaryRight}><Text style={styles.summaryValue}>{formatBRL(TOTAL)}</Text></View>
              </View>
              <View style={styles.summarySeparator}>
                <View style={styles.summarySeparatorLine} />
              </View>
              <View style={styles.summaryRow}>
                <View style={styles.summaryLeft}><Text style={styles.summaryLabel}>Restante</Text></View>
                <View style={styles.summaryRight}><Text style={styles.summaryValue}>{formatBRL(restante)}</Text></View>
              </View>
            </View>
            <View style={styles.formFieldWrap}>
              <View style={styles.formLabelContainer}><Text style={styles.formLabel}>Prolabore</Text></View>
              <View style={styles.input}
                >
                <TextInput
                  style={[styles.textInput, Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0 } as any) : null]}
                  value={focusedField === 'prolabore' ? prolaboreInput : (prolaboreInput === '' ? '' : currencyNoPrefix(maskCurrencyBRL(formatBRLValue(prolaboreNum))))}
                  onChangeText={(text) => { setProlaboreInput(text); }}
                  keyboardType={'default'}
                  placeholder="0,00"
                  placeholderTextColor="#91929E"
                  selectionColor="#1777CF"
                  cursorColor="#1777CF"
                  onFocus={() => { setFocusedField('prolabore'); }}
                  onBlur={() => { setFocusedField(null); const raw = (prolaboreInput || '').trim(); const n = parseCurrencyInput(raw); setProlaboreNum(Math.max(n, 0)); }}
                  onSubmitEditing={() => { const raw = (prolaboreInput || '').trim(); const n = parseCurrencyInput(raw); setProlaboreNum(Math.max(n, 0)); }}
                  accessibilityLabel="Campo de Prolabore"
                />
              </View>
            </View>
            <View style={styles.formFieldWrap}>
              <View style={styles.formLabelContainer}><Text style={styles.formLabel}>Êxito</Text></View>
              <View style={styles.input}
                >
                <TextInput
                  style={[styles.textInput, Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0 } as any) : null]}
                  value={focusedField === 'exito' ? exitoInput : (exitoInput === '' ? '' : currencyNoPrefix(maskCurrencyBRL(formatBRLValue(exitoNum))))}
                  onChangeText={(text) => { setExitoInput(text); }}
                  keyboardType={'default'}
                  placeholder="0,00"
                  placeholderTextColor="#91929E"
                  selectionColor="#1777CF"
                  cursorColor="#1777CF"
                  onFocus={() => { setFocusedField('exito'); }}
                  onBlur={() => { setFocusedField(null); const raw = (exitoInput || '').trim(); const n = parseCurrencyInput(raw); setExitoNum(Math.max(n, 0)); }}
                  onSubmitEditing={() => { const raw = (exitoInput || '').trim(); const n = parseCurrencyInput(raw); setExitoNum(Math.max(n, 0)); }}
                  accessibilityLabel="Campo de Êxito"
                />
              </View>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  summaryCard: {
    alignSelf: 'stretch',
    padding: 10,
    backgroundColor: 'rgba(23, 118, 207, 0.03)',
    borderRadius: 8,
    paddingVertical: 10,
    flexDirection: 'column',
    justifyContent: 'flex-start',
    ...
    marginTop: 25,
    ...(Platform.OS === 'web'
      ? ({ outlineStyle: 'solid', outlineWidth: 1, outlineColor: '#D8E0F0', outlineOffset: -1 } as any)
      : ({ borderWidth: 1, borderColor: '#D8E0F0' } as any)),
  },
  summaryRow: {
    alignSelf: 'stretch',
    borderRadius: 4,
...
export default NewAppointmentAgendaTypesPersonalize;
