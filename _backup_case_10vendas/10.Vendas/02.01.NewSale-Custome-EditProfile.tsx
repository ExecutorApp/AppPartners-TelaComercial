...
import { View, Text, StyleSheet, TouchableOpacity, Modal, TextInput, Platform, Image, ScrollView, TouchableWithoutFeedback, Animated } from 'react-native';
...
type NewKeymanPayload = EditProfileData;
...
interface NewKeymanProps {
  ...
  onLikeStatusChange?: (status: 'positive' | 'negative') => void;
}

const NewKeyman: React.FC<NewKeymanProps> = ({ visible, onClose, onSave, initialData, onLikeStatusChange }) => {
  ...
  const [operationAssetInput, setOperationAssetInput] = useState('');
  const [operationTaxCreditInput, setOperationTaxCreditInput] = useState('');
  const [operationTaxPlanningInput, setOperationTaxPlanningInput] = useState('');
  ...
  const sectionPositionsRef = useRef<{ localizacao: number; operacao: number }>({ localizacao: 0, operacao: 0 });

  const handleSectionLayout = (section: 'localizacao' | 'operacao') => (event: LayoutChangeEvent) => {
    const { y } = event.nativeEvent.layout;
    sectionPositionsRef.current = { ...sectionPositionsRef.current, [section]: y };
  };

  const handleTabPress = useCallback((tab: number) => {
    isScrollingProgrammatically.current = true;
    activeTabRef.current = tab;
    setActiveTab(tab);
    const sv = scrollViewRef.current;
    if (!sv) { isScrollingProgrammatically.current = false; return; }
    if (tab === 1) {
      sv.scrollTo({ y: 0, animated: true });
    } else if (tab === 2) {
      const offset = Math.max(0, sectionPositionsRef.current.localizacao - 10);
      sv.scrollTo({ y: offset, animated: true });
    } else if (tab === 3) {
  ...
  const handleScroll = useCallback((event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    let next = activeTabRef.current;
    if (scrollY >= Math.max(0, maxScroll - tolerance)) next = 3;
    else if (scrollY >= Math.max(0, sectionPositionsRef.current.localizacao - tolerance)) next = 2;
    else next = 1;
    if (next !== activeTabRef.current) { activeTabRef.current = next; setActiveTab(next); }
  }, []);
  ...
  const collectErrors = (data: typeof formData) => {
    ...
    if (wDigits.length > 0 && (wDigits.length < 10 || !validateWhatsApp(data.whatsapp).valid)) newErrors.whatsapp = 'WhatsApp inválido (DDD e comprimento).';
    ...
    const op1 = onlyDigits(currencyNoPrefix(data.operationAsset));
    if (op1.length === 0) newErrors.operationAsset = 'Valor patrimonial é obrigatório.';
    const op2 = onlyDigits(currencyNoPrefix(data.operationTaxCredit));
    if (op2.length === 0) newErrors.operationTaxCredit = 'Crédito tributário é obrigatório.';
    const op3 = onlyDigits(currencyNoPrefix(data.operationTaxPlanning));
    if (op3.length === 0) newErrors.operationTaxPlanning = 'Planejamento tributário é obrigatório.';

    return newErrors;
  };

  const isPositiveCurrency = (s: string): boolean => {
    const n = parseCurrencyInput(currencyNoPrefix(s));
    return n > 0;
  };

  const computeLikeStatus = (data: typeof formData): 'positive' | 'negative' => {
    const addressOk = !!data.address && data.address.trim().length > 0;
    const numberOk = !!data.number && data.number.trim().length > 0;
    const assetOk = isPositiveCurrency(data.operationAsset || '');
    const creditOk = isPositiveCurrency(data.operationTaxCredit || '');
    const planningOk = isPositiveCurrency(data.operationTaxPlanning || '');
    const allOk = addressOk && numberOk && assetOk && creditOk && planningOk;
    return allOk ? 'positive' : 'negative';
  };

  React.useEffect(() => {
    const status = computeLikeStatus(formData);
    onLikeStatusChange?.(status);
  }, [formData, onLikeStatusChange]);
  ...
  React.useEffect(() => {
    ...
    setFormData({
      ...
      operationAsset: initialData.operationAsset,
      operationTaxCredit: initialData.operationTaxCredit,
      operationTaxPlanning: initialData.operationTaxPlanning,
    });
    setOperationAssetInput('');
    setOperationTaxCreditInput('');
    setOperationTaxPlanningInput('');
    setErrors({});
    setTouched({});
    setSubmitted(false);
  }, [visible, initialData]);
  ...
  const handleCreate = () => {
    ...
    const payload: NewKeymanPayload = {
      ...
      operationAsset: formData.operationAsset,
      operationTaxCredit: formData.operationTaxCredit,
      operationTaxPlanning: formData.operationTaxPlanning,
    };
    if (personType === 'FISICA') {
  ...
  const sanitizeCurrencyTyping = (text: string): string => {
  ...
  const extractNumericFromCurrency = (s: string): string => {
  ...
  const currencyNoPrefix = (s: string): string => {
  ...
  const PlusSquareIcon = () => (
    <Svg width={40} height={35} viewBox="0 0 40 35" fill="none">
      <Rect x={0.25} y={0.25} width={39.5} height={34.5} rx={5.75} fill="#1777CF" stroke="#D8E0F0" strokeWidth={0.5} />
            ...
            <View style={styles.content}>
              ...
              <View style={styles.midBlock}>
                <View style={styles.stepRow}>
                  <TouchableOpacity
                    style={[styles.stepBox, activeTab === 1 ? styles.stepBoxActive : null]}
                    onPress={() => handleTabPress(1)}
                    accessibilityRole="button"
                    accessibilityLabel="Ir para Keyman"
                  >
                    <Text style={[styles.stepText, activeTab === 1 ? styles.stepTextActive : null]}>01</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.stepBox, activeTab === 2 ? styles.stepBoxActive : null]}
                    onPress={() => handleTabPress(2)}
                    accessibilityRole="button"
                    accessibilityLabel="Ir para Localização"
                  >
                    <Text style={[styles.stepText, activeTab === 2 ? styles.stepTextActive : null]}>02</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.stepBox, activeTab === 3 ? styles.stepBoxActive : null]}
                    onPress={() => handleTabPress(3)}
                    accessibilityRole="button"
                    accessibilityLabel="Ir para Operação"
                  >
                    <Text style={[styles.stepText, activeTab === 3 ? styles.stepTextActive : null]}>03</Text>
                  </TouchableOpacity>
                </View>
              ...
              >
                <View style={styles.keymanSection}>
                  <View style={styles.keymanHeaderRow}>
                    ...
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                      accessibilityLabel="Adicionar Keyman"
                    >
                      <PlusSquareIcon />
                    </TouchableOpacity>
                  </View>
                  <View style={styles.keymanSelectRow}>
                    <Text style={styles.keymanSelectText}>João Fernando</Text>
                    <ChevronDownIcon />
                  </View>
                </View>
                ...
                {/* Localização */}
                <View onLayout={handleSectionLayout('localizacao')}>
                ...
                <View style={styles.operationSection} onLayout={handleSectionLayout('operacao')}>
                  <View style={styles.operationHeaderWrap}>
                    <Text style={styles.operationHeaderTitle}>Operação:</Text>
                  </View>
                <View style={styles.operationFieldWrap}>
                  <View style={styles.operationFieldLabelWrap}>
                    <Text style={styles.operationFieldLabel}>Valor patrimonial *</Text>
                  </View>
                  <View style={[styles.input, focusedField === 'operationAsset' ? styles.inputFocused : null, styles.currencyInput]}>
                    <TextInput
                      style={[styles.currencyTextInput, Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0 } as any) : null]}
                      value={focusedField === 'operationAsset' ? operationAssetInput : currencyNoPrefix(formData.operationAsset)}
                      onChangeText={(text) => {
                        setOperationAssetInput(sanitizeCurrencyTyping(text));
                      }}
                      placeholder="10.000.000,00"
                      keyboardType={Platform.OS === 'ios' ? 'decimal-pad' : 'default'}
                      placeholderTextColor="#91929E"
                      onFocus={() => {
                        setFocusedField('operationAsset');
                        setOperationAssetInput(extractNumericFromCurrency(formData.operationAsset));
                      }}
                      onBlur={() => {
                        setFocusedField(null);
                        const raw = operationAssetInput.trim();
                        const n = parseCurrencyInput(raw);
                        if (!raw || n <= 0) {
                          setFormData({ ...formData, operationAsset: '' });
                        } else {
                          const formatted = formatBRLValue(n);
                          setFormData({ ...formData, operationAsset: formatted });
                        }
                        setTouched({ ...touched, operationAsset: true });
                      }}
                      selectionColor="#1777CF"
                      cursorColor="#1777CF"
                      accessibilityLabel="Campo de valor patrimonial"
                    />
                  </View>
                  {(errors.operationAsset && (submitted || touched.operationAsset)) ? (
                    <Text style={styles.errorText}>{errors.operationAsset}</Text>
                  ) : null}
                </View>
                <View style={styles.operationFieldWrap}>
                  <View style={styles.operationFieldLabelWrap}>
                    <Text style={styles.operationFieldLabel}>Crédito tributário *</Text>
                  </View>
                  <View style={[styles.input, focusedField === 'operationTaxCredit' ? styles.inputFocused : null, styles.currencyInput]}>
                    <TextInput
                      style={[styles.currencyTextInput, Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0 } as any) : null]}
                      value={focusedField === 'operationTaxCredit' ? operationTaxCreditInput : currencyNoPrefix(formData.operationTaxCredit)}
                      onChangeText={(text) => {
                        setOperationTaxCreditInput(sanitizeCurrencyTyping(text));
                      }}
                      placeholder="200.000,00"
                      keyboardType={Platform.OS === 'ios' ? 'decimal-pad' : 'default'}
                      placeholderTextColor="#91929E"
                      onFocus={() => {
                        setFocusedField('operationTaxCredit');
                        setOperationTaxCreditInput(extractNumericFromCurrency(formData.operationTaxCredit));
                      }}
                      onBlur={() => {
                        setFocusedField(null);
                        const raw = operationTaxCreditInput.trim();
                        const n = parseCurrencyInput(raw);
                        if (!raw || n <= 0) {
                          setFormData({ ...formData, operationTaxCredit: '' });
                        } else {
                          const formatted = formatBRLValue(n);
                          setFormData({ ...formData, operationTaxCredit: formatted });
                        }
                        setTouched({ ...touched, operationTaxCredit: true });
                      }}
                      selectionColor="#1777CF"
                      cursorColor="#1777CF"
                      accessibilityLabel="Campo de crédito tributário"
                    />
                  </View>
                  {(errors.operationTaxCredit && (submitted || touched.operationTaxCredit)) ? (
                    <Text style={styles.errorText}>{errors.operationTaxCredit}</Text>
                  ) : null}
                </View>
                <View style={styles.operationFieldWrap}>
                  <View style={styles.operationFieldLabelWrap}>
                    <Text style={styles.operationFieldLabel}>Planejamento tributário *</Text>
                  </View>
                  <View style={[styles.input, focusedField === 'operationTaxPlanning' ? styles.inputFocused : null, styles.currencyInput]}>
                    <TextInput
                      style={[styles.currencyTextInput, Platform.OS === 'web' ? ({ outlineStyle: 'none', outlineWidth: 0 } as any) : null]}
                      value={focusedField === 'operationTaxPlanning' ? operationTaxPlanningInput : currencyNoPrefix(formData.operationTaxPlanning)}
                      onChangeText={(text) => {
                        setOperationTaxPlanningInput(sanitizeCurrencyTyping(text));
                      }}
                      placeholder="200.000,00"
                      keyboardType={Platform.OS === 'ios' ? 'decimal-pad' : 'default'}
                      placeholderTextColor="#91929E"
                      onFocus={() => {
                        setFocusedField('operationTaxPlanning');
                        setOperationTaxPlanningInput(extractNumericFromCurrency(formData.operationTaxPlanning));
                      }}
                      onBlur={() => {
                        setFocusedField(null);
                        const raw = operationTaxPlanningInput.trim();
                        const n = parseCurrencyInput(raw);
                        if (!raw || n <= 0) {
                          setFormData({ ...formData, operationTaxPlanning: '' });
                        } else {
                          const formatted = formatBRLValue(n);
                          setFormData({ ...formData, operationTaxPlanning: formatted });
                        }
                        setTouched({ ...touched, operationTaxPlanning: true });
                      }}
                      selectionColor="#1777CF"
                      cursorColor="#1777CF"
                      accessibilityLabel="Campo de planejamento tributário"
                    />
                  </View>
                  {(errors.operationTaxPlanning && (submitted || touched.operationTaxPlanning)) ? (
                    <Text style={styles.errorText}>{errors.operationTaxPlanning}</Text>
                  ) : null}
                </View>
              </View>
              </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Linha de cabeçalho (título + ações)
  headerRow: {
    flexDirection: 'row',
  ...
  // Cabeçalho de Keyman (título + botão adicionar)
  keymanHeaderRow: {
    alignSelf: 'stretch',
    justifyContent: 'flex-start',
    alignItems: 'center',
  ...
  // Barra de ações fixa (Cancelar/Criar)
  actionsBar: {
    position: 'absolute',
    left: 20,
    right: 20,
  ...
  // Botões
  // Botão fechar (cabeçalho)
  closeButton: {
    padding: 4,
  },
  // Botão adicionar Keyman
  plusButton: {
    width: 40,
    height: 35,
    alignItems: 'center',
  ...
  // Estado ativo do bloco de passo
  stepBoxActive: {
    backgroundColor: '#1777CF',
  },
  // Botão primário (Criar)
  primaryButton: {
    flex: 1,
    minWidth: 0,
    backgroundColor: '#1777CF',
  ...
  // Título da seção Operação
  operationHeaderTitle: {
    color: '#0A1629',
    fontSize: 16,
    fontFamily: 'Inter_600SemiBold',
  },
  // Label dos campos de Operação
  operationFieldLabel: {
    flex: 1,
    alignSelf: 'stretch',
    color: '#7D8592',
  ...
  // Títulos principais de seção (Dados pessoais / Localização)
  sectionTitle: {
    fontSize: 16,
    fontFamily: 'Inter_700Bold',
    color: '#3A3F51',
  ...
  // Texto dos itens de estado (UF)
  stateItemText: {
    fontSize: 14,
    fontFamily: 'Inter_400Regular',
    color: '#3A3F51',
  },
  // Texto dos passos (01/02/03)
  stepText: {
    textAlign: 'center',
    color: '#3A3F51',
    fontSize: 14,
