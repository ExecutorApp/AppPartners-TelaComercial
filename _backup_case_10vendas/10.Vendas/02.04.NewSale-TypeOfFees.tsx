...
const NewAppointmentAgendaTypes: React.FC<NewAppointmentAgendaTypesProps> = ({ visible, onClose, onBack, onNext, onSelectAgendaType, summaries, onSelectStep, onUpdateSummary, maxAccessibleStep = 1, transitionDirection = 'forward', embedded = false, registerNextHandler, onCanProceedChange }) => {
  ...
  const agendaTypes: FeeItem[] = useMemo(() => [
    { id: 'prolabore', number: '01', name: 'Prolabore', value: 120000 },
    { id: 'prolabore_success', number: '02', name: 'Prolabore + Êxito', value: 135000 },
    { id: 'success', number: '03', name: 'Êxito', value: 160000 },
  ], []);
  ...
  useEffect(() => {
    ...
    if (summ.includes('prolabore') && (summ.includes('êxito') || summ.includes('exito'))) fromSummary = 'prolabore_success';
    else if (summ.includes('prolabore')) fromSummary = 'prolabore';
    else if (summ.includes('êxito') || summ.includes('exito')) fromSummary = 'success';
    if (fromSummary) {
      setSelectedType(fromSummary);
      onSelectAgendaType?.(fromSummary);
      const found = agendaTypes.find(t => t.id === fromSummary);
      if (found) onUpdateSummary?.(5, found.name);
    } else {
      // Seleciona "Prolabore" por padrão
      const defaultType = 'prolabore';
      setSelectedType(defaultType);
      onSelectAgendaType?.(defaultType);
      const found = agendaTypes.find(t => t.id === defaultType);
      if (found) onUpdateSummary?.(5, found.name);
    }
  }, [visible, agendaTypes]);
          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {agendaTypes.map((t, idx) => (
              <View key={t.id}>
                ...
                {t.id === 'prolabore_success' ? (
                  <View style={styles.detailsContainer}>
                    <View style={styles.detailsTopDivider} />
                    <View style={styles.detailsItem}>
                      <Text style={styles.feeName}>Prolabore</Text>
                      <Text style={styles.feeValue}>{formatBRL(40000)}</Text>
                    </View>
                    <View style={[styles.detailsItem, styles.detailsBetweenSpacer]}>
                      <Text style={styles.feeName}>Êxito</Text>
                      <Text style={styles.feeValue}>{formatBRL(95000)}</Text>
                    </View>
                  </View>
                ) : null}
